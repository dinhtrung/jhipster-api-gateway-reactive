...
import com.ft.service.dto.UploadResult;
import com.ft.web.rest.errors.UploadFailedException;
...
public class UploadResource {
    ...
    public UploadResource(S3AsyncClient s3client, S3Properties s3config) {
    ...
    public Mono<ResponseEntity<UploadResult>> uploadHandler(@RequestHeader HttpHeaders headers, @RequestBody Flux<ByteBuffer> body) {
        ...
        log.debug("[I95] uploadHandler: mediaType{}, length={}", mediaType, length);
    ...
    protected Mono<String> saveFile(String bucket, FilePart part) {
        ...
        log.debug("[I137] saveFile: filekey={}, filename={}", filekey, part.filename());
        ...
        return Mono
          ...
          .flatMapMany(response -> {
              ...
              log.debug("[I183] uploadId={}", response.uploadId());
              return part.content();
          })
          .bufferUntil(buffer -> {
              uploadState.buffered += buffer.readableByteCount();
              if ( uploadState.buffered >= s3config.getMultipartMinPartSize() ) {
                  log.debug("[I173] bufferUntil: returning true, bufferedBytes={}, partCounter={}, uploadId={}", uploadState.buffered, uploadState.partCounter, uploadState.uploadId);
          ...
          .reduce(uploadState,(state,completedPart) -> {
              log.debug("[I188] completed: partNumber={}, etag={}", completedPart.partNumber(), completedPart.eTag());
    ...
    private static ByteBuffer concatBuffers(List<DataBuffer> buffers) {
        log.debug("[I198] creating BytBuffer from {} chunks", buffers.size());
        ...
        log.debug("[I208] partData: size={}", partData.capacity());
        return partData;
    ...
    private Mono<CompletedPart> uploadPart(UploadState uploadState, ByteBuffer buffer) {
        ...
        log.debug("[I218] uploadPart: partNumber={}, contentLength={}",partNumber, buffer.capacity());
        ...
        return Mono
          ...
          .map(uploadPartResult -> {              
              ...
              log.debug("[I230] uploadPart complete: part={}, etag={}",partNumber,uploadPartResult.eTag());
    ...
    private Mono<CompleteMultipartUploadResponse> completeUpload(UploadState state) {        
        log.debug("[I202] completeUpload: bucket={}, filekey={}, completedParts.size={}", state.bucket, state.filekey, state.completedParts.size());        
